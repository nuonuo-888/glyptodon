import { renderers } from './renderers.mjs';
import { c as createExports, s as serverEntrypointModule } from './chunks/_@astrojs-ssr-adapter_DLus0CCi.mjs';
import { manifest } from './manifest_cOSmR6jT.mjs';

const serverIslandMap = new Map();;

const _page0 = () => import('./pages/404.astro.mjs');
const _page1 = () => import('./pages/about.astro.mjs');
const _page2 = () => import('./pages/finds/_page_.astro.mjs');
const _page3 = () => import('./pages/finds.astro.mjs');
const _page4 = () => import('./pages/now.astro.mjs');
const _page5 = () => import('./pages/posts/_page_.astro.mjs');
const _page6 = () => import('./pages/posts/_slug_.astro.mjs');
const _page7 = () => import('./pages/posts.astro.mjs');
const _page8 = () => import('./pages/robots.txt.astro.mjs');
const _page9 = () => import('./pages/rss.xml.astro.mjs');
const _page10 = () => import('./pages/index.astro.mjs');
const pageMap = new Map([
    ["src/pages/404.md", _page0],
    ["src/pages/about.md", _page1],
    ["src/pages/finds/[page].astro", _page2],
    ["src/pages/finds/index.astro", _page3],
    ["src/pages/now.md", _page4],
    ["src/pages/posts/[page].astro", _page5],
    ["src/pages/posts/[slug].astro", _page6],
    ["src/pages/posts/index.astro", _page7],
    ["src/pages/robots.txt.ts", _page8],
    ["src/pages/rss.xml.js", _page9],
    ["src/pages/index.astro", _page10]
]);

const _manifest = Object.assign(manifest, {
    pageMap,
    serverIslandMap,
    renderers,
    actions: () => import('./_noop-actions.mjs'),
    middleware: () => import('./_noop-middleware.mjs')
});
const _args = undefined;
const _exports = createExports(_manifest);
const __astrojsSsrVirtualEntry = _exports.default;
const _start = 'start';
if (Object.prototype.hasOwnProperty.call(serverEntrypointModule, _start)) {
	serverEntrypointModule[_start](_manifest, _args);
}

export { __astrojsSsrVirtualEntry as default, pageMap };
